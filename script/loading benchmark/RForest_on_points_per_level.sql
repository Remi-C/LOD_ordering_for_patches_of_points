...
,count_per_class as (
	SELECT dominant_simplified_class , count(*)
	FROM  patch_to_use
		WHERE (dominant_simplified_class=5 AND random() <0.90) = false
...
SELECT  r.* 
FROM array_agg,rc_random_forest_cross_valid_per_class(gid,f1,f2,f3,f4,gt_class,proba,10,100) as r ;
