...
--▓▒░Creating a dedicated postgres schema▓▒░--
	...
	DROP SCHEMA IF EXISTS benchmark_cassette_2013;
	--creating the schema
	CREATE SCHEMA benchmark_cassette_2013;

	
--▓▒░Creating the XML schema corresponding to points acquired by a Riegl Laser and a Velodyn Laser▓▒░--

	--ALTER TABLE pointcloud_formats RENAME COLUMN nom_schema TO schema_name
	----
	...
	--Creating the XML  schema for points acquired by a riegl laser:
			...
			INSERT INTO pointcloud_formats (pcid, srid, schema_name) VALUES (6, 932011,'Riegl_Benchmark_IGN');--On crée un nouveau schema
			--Filling the entry
...
--▓▒░Creating tables which will contain patches ▓▒░--
	...
	--Creating table for riegl laser, for space partitionning
		DROP TABLE IF EXISTS benchmark_cassette_2013.riegl_pcpatch_space CASCADE;
		CREATE TABLE benchmark_cassette_2013.riegl_pcpatch_space(
			gid SERIAL PRIMARY KEY,
			patch PCPATCH(6)
		);
	ALTER TABLE benchmark_cassette_2013.riegl_pcpatch_space ALTER COLUMN patch SET STORAGE EXTERNAL;
